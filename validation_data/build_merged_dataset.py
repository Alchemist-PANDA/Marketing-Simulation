"""
Merge original (wave 1, 2026-07-11) + fresh (wave 2, 2026-07-12) TikTok ads
into ONE corpus and re-split by brand-hash across the combined pool. This
is the actual fix for the fresh-data generalization failure: instead of
training on a single scrape wave and being blindsided by a new one, both
train AND holdout now contain ads from both waves, forcing the model to
learn wave-invariant signal instead of wave-specific artifacts.
"""
import csv
import hashlib
import random
...
for a in wave1 + wave2:
    by_id[a["id"]] = a  # wave2 wins on collision (shouldn't happen, already deduped)

ads = list(by_id.values())
print(f"Merged unique ads: {len(ads)} (wave1={len(wave1)}, wave2={len(wave2)})")
